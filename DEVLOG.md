...
| 20. jun 2026 | (denne commit) | **Iterasjon 4** — Heatmap-side på `/heatmap` som visualiserer kartleggings-tetthet i Oslo via GBIF-observasjoner. Bruker leaflet.heat + ny `scripts/build-heatmap-data.js` (kjøres med `npm run build:heatmap-data`, ~5 min). Adresserer R7-innspill: «sier ikke noe om hva vi IKKE får informasjon om» — heatmapet viser eksplisitt hvor det er kunnskapshull |
