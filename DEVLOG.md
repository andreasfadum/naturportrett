...
| 21. jun 2026 | (denne commit) | **Iterasjon 12** — Token- og kostnadssporing for Claude-kall. Tidligere kastet vi `response.usage` fra Anthropic SDK — nå loggføres hvert kall som én linje i `claude-usage.jsonl` på det persistente volumet med tidspunkt, IP, kontekst (`portrait:naturportrett` / `vurdering` / osv.), modell, input/output/cache-tokens og estimert USD. Modellpriser i `server/usage/index.js`: sonnet $3/$15, opus $15/$75, haiku $1/$5 per million tokens. Ny admin-side `/admin/usage` viser totalt + i dag + siste 7 dager med fordeling per modell og per portretttype, sessions (unik IP innenfor 30 min) og snitt USD/kall per session. Disclaimer: tall er estimater — fasiten ligger i Anthropic Console. |
