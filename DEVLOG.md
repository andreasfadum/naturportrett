...
| 21. jun 2026 | (denne commit) | **Iterasjon 11** — Feedback-routing til e-post via Resend. Tidligere lagret tilbakemeldinger seg bare på Railway-volumet og krevde admin-side-tilgang for å leses. Nå sendes hver tilbakemelding også til `FEEDBACK_RECIPIENT_EMAIL` (default `andreas.haugstad@pbe.oslo.kommune.no`) med emne-prefiks `[Naturportrett tilbakemelding]` for Outlook-filtrering. Sendt fra `onboarding@resend.dev` så domeneverifisering ikke kreves — fungerer så lenge mottaker-adressen er den samme som Resend-kontoens registrerte e-post. `replyTo` settes til brukerens e-post hvis oppgitt, slik at man kan svare direkte. Fire-and-forget — venter ikke på Resend før klient får OK. Hvis `RESEND_API_KEY` mangler hopper over e-post (logges som warn), lagring fortsetter. |
